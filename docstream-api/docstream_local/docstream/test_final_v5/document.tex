...
\bibitem{ref1} Reference 1 text.
\bibitem{ref15} Reference 15 text.
\bibitem{ref16} Reference 16 text.
\bibitem{ref11} Reference 11 text.
\bibitem{ref12} Reference 12 text.
\bibitem{ref3} Reference 3 text.
\bibitem{ref9} Reference 9 text.
\bibitem{ref17} Reference 17 text.
\bibitem{ref2} Reference 2 text.
\bibitem{ref7} Reference 7 text.
\bibitem{ref5} Reference 5 text.
\bibitem{ref4} Reference 4 text.
\bibitem{ref18} Reference 18 text.
\bibitem{ref19} Reference 19 text.
\bibitem{ref8} Reference 8 text.
\bibitem{ref6} Reference 6 text.
\bibitem{ref14} Reference 14 text.
\bibitem{ref13} Reference 13 text.
\bibitem{ref20} Reference 20 text.
\bibitem{ref10} Reference 10 text.
\end{thebibliography}

\subsection{Data Processing and Flow}
System automates workflow to provide smooth AR experience:
\begin{enumerate}
    \item User picks a piece of furniture or input a custom 2D image.
    \item System forwards the image to the Meshy AI API to create 3D model \cite{ref5}.
    \item 3D model is saved in Firebase and reused after being created once.
    \item Users can view and navigate through the 3D model in an AR environment using ARCore \cite{ref2}.
...
\begin{itemize}
    \item Flutter: Cross-platform implementation of the app \cite{ref3}.
    \item Meshy AI API: 2D image to 3D model conversion \cite{ref5}.
    \item ARCore SDK: AR rendering and real-world interaction management \cite{ref2}.
    \item Firebase: Authentication and database management \cite{ref4}.
\end{itemize}

\subsection{Challenges and Optimizations}
Technical issues faced while deploying:
\begin{enumerate}
    \item ARCore Installation Issues: Initial problems with installing ARCore modules and rendering AR without hiccups \cite{ref17}.
    \item CPU Usage on Emulator: AR performance duration on PC emulator was poor; usage of actual Android devices showed better performance \cite{ref9}.
    \item 3D Model Storage Optimization: Firebase had been set up to cache pre-rendered models in order to prevent unnecessary API calls and enhance the user experience \cite{ref19}.
\end{enumerate}
...
\begin{figure}[htbp]
    \centering
    \rule{0.8\columnwidth}{5cm} % Placeholder for the figure
    \caption{Login system screen.}
    \label{fig:login_screen}
\end{figure}

Once authenticated, users are presented with a sample products screen (Figure \ref{fig:products_screen}) that displays a catalog of furniture items, organized by categories. This screen is critical for guiding the user experience by enabling easy navigation through available options. The interface is designed with clarity in mind, ensuring that users can effortlessly browse through different products and select items of interest.

\begin{figure}[htbp]
    \centering
    \rule{0.8\columnwidth}{5cm} % Placeholder for the figure
    \caption{Sample products screen.}
    \label{fig:products_screen}
\end{figure}

When a user selects a product or opts to upload a new image, the system transitions to the 2D-to-3D model generation screen (Figure \ref{fig:generation_screen}). Here, the application sends the image to the Meshy AI API, which processes it to generate a 3D model. Although this conversion process takes approximately 2--3 minutes, subsequent accesses to the same model are expedited by caching in Firebase Cloud Storage. This functionality is crucial for reducing redundant processing and enhancing the overall efficiency of the system.

\begin{figure}[htbp]
    \centering
    \rule{0.8\columnwidth}{5cm} % Placeholder for the figure
    \caption{2D-to-3D model generation screen.}
    \label{fig:generation_screen}
\end{figure}

Following model generation, the application presents the 3D model rendering screen (Figure \ref{fig:rendering_screen}). This module demonstrates how the system successfully converts a 2D image into a visually detailed 3D model, ready for interaction. The rendering quality is evaluated based on visual fidelity and the accuracy of model details, ensuring that the output meets user expectations.

\begin{figure}[htbp]
    \centering
    \rule{0.8\columnwidth}{5cm} % Placeholder for the figure
    \caption{3D model rendering screen.}
    \label{fig:rendering_screen}
\end{figure}
